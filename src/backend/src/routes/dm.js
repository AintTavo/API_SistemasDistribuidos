...
router.post('/stories', async (req, res) => {
  const s = req.body || {};
  if (!(await isDM(req.user.id, s.party_id))) return res.status(403).json({ error: 'Solo el DM de la mesa' });
  const r = await query(
    'INSERT INTO stories (party_id, title, content) VALUES ($1,$2,$3) RETURNING *',
...
router.put('/stories/:id', async (req, res) => {
  ...
  const owner = await query(
    'SELECT p.dm_user_id FROM stories st JOIN parties p ON p.id=st.party_id WHERE st.id=$1',
    [req.params.id]
  );
  if (owner.rowCount === 0 || owner.rows[0].dm_user_id !== req.user.id) return res.status(403).json({ error: 'Solo el DM' });
  const r = await query(
