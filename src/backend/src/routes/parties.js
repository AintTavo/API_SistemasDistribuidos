...
// Listar las parties donde soy DM o miembro.
router.get('/', async (req, res) => {
  const r = await query(
    `SELECT p.*, (p.dm_user_id = $1) AS is_dm,
            (SELECT count(*) FROM party_members pm WHERE pm.party_id = p.id) AS member_count
     FROM parties p
     WHERE p.dm_user_id = $1
        OR p.id IN (SELECT party_id FROM party_members WHERE user_id = $1)
...
// Crear una mesa (quien la crea es el DM).
router.post('/', async (req, res) => {
  const { name } = req.body || {};
  if (!name) return res.status(400).json({ error: 'name requerido' });
  const code = randomCode();
  const r = await query(
    'INSERT INTO parties (name, dm_user_id, join_code) VALUES ($1,$2,$3) RETURNING *',
    [name, req.user.id, code]
  );
  await query(
    'INSERT INTO party_members (party_id, user_id, role) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING',
    [r.rows[0].id, req.user.id, 'dm']
  );
  res.status(201).json(r.rows[0]);
});

// Unirse a una mesa con su código (modo usuario).
