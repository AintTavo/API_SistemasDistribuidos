...
const { abilityMod, rollDie, isMemberOrDM } = require('./helpers');

// Realtime del MODO MAZMORRA.
// El DM inicia la sala y ésta se "activa" cuando otros usuarios se unen.
function attachSockets(httpServer) {
  const io = new Server(httpServer, { cors: { origin: '*' } });

  // Autenticación del socket vía JWT (inicio de sesión obligatorio).
  io.use((socket, next) => {
  ...
  io.on('connection', (socket) => {
    socket.on('join_session', async ({ sessionId }) => {
      ...
      // Acceso solo para DM y miembros de esa party.
      if (!(await isMemberOrDM(socket.user.id, session.party_id))) {
        return socket.emit('error_msg', 'Sin acceso a esta sala');
      }
      ...
      // Si un jugador (no DM) se une, la sala pasa a "active".
    ...
    // El DM dibuja la mazmorra; se propaga y persiste el trazo.
    socket.on('draw', async ({ sessionId, stroke }) => {
      if (!socket.data.isDM) return; // solo el DM dibuja
      await query(
        "UPDATE dungeon_sessions SET canvas_data = canvas_data || $1::jsonb WHERE id=$2",
    ...
    // Gestión de turnos (orden de iniciativa).
    ...
    // Tiradas de habilidad: el servidor tira el d20 (anti-trampa).
    socket.on('skill_check', async ({ sessionId, actor, ability, score, proficiency, dc }) => {
      ...
      if (!(await isMemberOrDM(socket.user.id, s.rows[0].party_id))) return;
